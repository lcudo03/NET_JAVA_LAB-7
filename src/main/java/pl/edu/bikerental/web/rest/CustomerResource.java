...
import pl.edu.bikerental.service.CustomerService;
...
/**
 * REST controller for managing {@link pl.edu.bikerental.domain.Customer}.
 */
@RestController
...
public class CustomerResource {
    ...
    public Mono<ResponseEntity<CustomerDTO>> createCustomer(@Valid @RequestBody CustomerDTO customerDTO) throws URISyntaxException {
        ...
        return customerService.save(customerDTO).map(result -> {
            try {
    ...
    public Mono<ResponseEntity<CustomerDTO>> updateCustomer(
        ...
        return customerRepository.existsById(id).flatMap(exists -> {
            if (!exists) {
    ...
    public Mono<ResponseEntity<List<CustomerDTO>>> getAllCustomers(
        ...
        return customerService
            ...
            .zipWith(customerService.findByCriteria(criteria, pageable).collectList())
    ...
    public Mono<ResponseEntity<Long>> countCustomers(CustomerCriteria criteria) {
        ...
        return customerService.countByCriteria(criteria).map(count -> ResponseEntity.status(HttpStatus.OK).body(count));
    }
