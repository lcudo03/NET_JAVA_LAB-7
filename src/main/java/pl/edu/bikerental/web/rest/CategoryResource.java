...
import pl.edu.bikerental.domain.criteria.CategoryCriteria;
import pl.edu.bikerental.repository.CategoryRepository;
import pl.edu.bikerental.service.CategoryService;
...
/**
 * REST controller for managing {@link pl.edu.bikerental.domain.Category}.
 */
@RestController
...
public class CategoryResource {
    ...
    public Mono<ResponseEntity<CategoryDTO>> createCategory(@Valid @RequestBody CategoryDTO categoryDTO) throws URISyntaxException {
        ...
        return categoryService.save(categoryDTO).map(result -> {
            try {
    ...
    public Mono<ResponseEntity<CategoryDTO>> updateCategory(
        ...
        return categoryRepository.existsById(id).flatMap(exists -> {
            if (!exists) {
