...
import { sampleWithRequiredData } from '../bike.test-samples';
...
describe('Bike Management Component', () => {
  ...
  let comp: Bike;
  let fixture: ComponentFixture<Bike>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Bike);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 9820 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.bikes()[0]).toEqual(expect.objectContaining({ id: 9820 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 9820 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.bikes()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 9820 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to bikeService', () => {
      ...
      vitest.spyOn(service, 'getBikeIdentifier');
      const id = comp.trackId(entity);
      expect(service.getBikeIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.bikesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.bikesParams()).toMatchObject(expect.objectContaining({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] }));
  });
