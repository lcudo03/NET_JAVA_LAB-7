...
import { sampleWithRequiredData } from '../customer-profile.test-samples';
...
describe('CustomerProfile Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 31508 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.customerProfiles()[0]).toEqual(expect.objectContaining({ id: 31508 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 31508 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.customerProfiles()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 31508 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to customerProfileService', () => {
      const entity = { id: 31508 };
      vitest.spyOn(service, 'getCustomerProfileIdentifier');
      const id = comp.trackId(entity);
      expect(service.getCustomerProfileIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.customerProfilesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.customerProfilesParams()).toMatchObject(
