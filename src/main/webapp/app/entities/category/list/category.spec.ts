...
describe('Category Management Component', () => {
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 6752 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    successReq.flush([{ id: 6752 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.categoriesParams()).toMatchObject(expect.objectContaining({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] }));
  });
